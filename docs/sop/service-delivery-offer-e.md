...
| Licensing Sales | growth_hacker | License sales, pricing |
...
| Developer Experience | platform_engineer | Onboarding flow, CLI ergonomics |
...
1. **Lead** — `head_of_developer_relations` captures lead; `product_marketing_manager`
   qualifies.
2. **Demo & Proposal** — `growth_hacker` demonstrates the platform; sends
...
5. **Platform Delivery** — `platform_engineer` packages a deployable
...
6. **Training** — `head_of_developer_relations` delivers 2-hour onboarding session
   (live + recording). `platform_engineer` provides CLI walkthrough.
7. **Ongoing Support** — `customer_success_owner` handles ticket escalation;
   `platform_engineer` monitors feedback for platform improvements.
...
2. The `senior_frontend_engineer`/`product_designer` website configs become starter templates.
...
- `platform_engineer` tracks time-to-first-success metric.

## 8. Post-Delivery

- 30-day intensive support window, then standard monthly support.
- `head_of_developer_relations` checks in at 7, 30, and 90 days.
- `growth_hacker` tracks licensee expansion (additional agents, modules).
