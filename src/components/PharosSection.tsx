    ...
    <section id="publications" className={`py-28 px-4 sm:px-8 max-w-7xl mx-auto w-full border-t relative overflow-hidden ${
      ...
      <div className="relative z-10 grid grid-cols-1 lg:grid-cols-12 gap-8 items-end mb-14">
        <div className="lg:col-span-8 space-y-4">
          <div className="inline-flex items-center gap-2.5 px-3.5 py-1.5 rounded-full border border-ls-cyan/40 bg-gradient-to-r from-ls-cyan/15 via-ls-red/10 to-transparent text-ls-cyan font-body text-[11px] tracking-widest shadow-sm">
            <span className="w-2 h-2 rounded-full bg-ls-cyan shadow-[0_0_8px_rgba(230,57,70,1)] animate-pulse" />
          ...
          <h2 className={`text-4xl sm:text-6xl font-black tracking-tight font-display leading-[1.05] ${
            ...
            <span className={isLight
              ? 'text-transparent bg-clip-text bg-gradient-to-r from-ls-cyan via-ls-red to-ls-navy'
              : 'text-transparent bg-clip-text bg-gradient-to-r from-ls-cyan via-ls-red to-ls-red/10'
            }>
              THOUGHT LEADERSHIP
            </span>
          </h2>
            ...
            <div className="relative z-10 space-y-3">
              <div className="flex items-center justify-between border-b border-ls-cyan/20 pb-2.5">
                <div className="flex items-center gap-2">
                  ...
                  <span className="font-body text-xs font-bold tracking-wider text-ls-cyan">
                    PHAROS OPTICAL BEACON
                  </span>
                </div>
                <span className="text-[10px] font-body px-2 py-0.5 rounded-full bg-ls-cyan/20 text-ls-cyan border border-ls-cyan/30 font-bold">
                  RADIANT
                </span>
              </div>
              ...
              <div className="space-y-1.5 text-[11px] font-body">
                <div className="flex justify-between">
                  <span className="text-ls-grey-light-text">Isle Coordinates:</span>
                  <span className="font-bold text-ls-cyan">31.2140 N, 29.8850 E</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-ls-grey-light-text">Optical Reach:</span>
                  <span className="font-bold text-ls-cyan">300 Stadia (~55 km)</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-ls-grey-light-text">Emission Spectrum:</span>
                  <span className="font-bold text-ls-red">589nm Solar Amber</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-ls-grey-light-text">Guiding Purpose:</span>
                  <span className={`font-bold ${isLight ? 'text-ls-navy' : 'text-ls-white'}`}>Board-Level Clarity</span>
                </div>
              </div>
            </div>
          </div>
        ...
        {whitepapers.map((wp, wIdx) => (
          <div
            ...
            className={`p-6 sm:p-7 rounded-3xl flex flex-col justify-between space-y-6 transition-all group relative overflow-hidden ${
              isLight
                ? 'hardware-chassis-light hover:border-ls-cyan text-ls-navy'
                : 'hardware-chassis-dark hover:border-ls-cyan/60 text-ls-grey-light-text'
            }`}
          >
            ...
            <div className="space-y-3.5 pt-1">
              ...
              <div className="flex items-center justify-between text-[11px] font-body border-b border-ls-navy/10 dark:border-ls-white/10 pb-2.5">
                <span className="px-2.5 py-0.5 rounded-full bg-ls-cyan/15 text-ls-cyan border border-ls-cyan/30 font-bold">
                  {wp.tome}
                </span>
                <span className="font-semibold text-ls-grey-light-text">{wp.readTime}</span>
              </div>

              {/* Subtitle & Tag */}
              <div className="flex items-center justify-between text-[10px] font-body">
                <span className="text-ls-red font-bold">{wp.tag}</span>
                <span className="text-ls-grey-light-text">{wp.greekNumeral}</span>
              </div>
              ...
              {/* Epigraph / Quote */}
              <blockquote className="text-xs italic border-l-2 pl-3 py-0.5 border-ls-cyan text-ls-grey-light-text">
                "{wp.epigraph}"
              </blockquote>

              {/* Detailed Description */}
              <p className="text-justify text-xs leading-relaxed text-ls-grey-light-text">
                {wp.desc}
              </p>
            </div>
            ...
            <div className={`pt-4 border-t flex items-center justify-between text-xs ${
              isLight ? 'border-ls-navy/10' : 'border-ls-white/10'
            }`}>
              <span className="text-[10px] font-body font-medium text-ls-grey-light-text">
                {wp.downloads}
              </span>
              <button
                onClick={() => onRequestBriefing(`Request Pharos Treatise: ${wp.tome} - ${wp.title}`)}
                className={`inline-flex items-center gap-1.5 px-3 py-1.5 rounded-xl text-xs font-body font-bold text-ls-cyan hover:text-ls-cyan cursor-pointer transition-all ${
