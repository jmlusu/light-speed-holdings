  ...
  return (
    <header className="fixed top-4 sm:top-6 left-0 right-0 z-50 flex justify-center px-3 sm:px-4 pointer-events-none">
      <div className="w-full max-w-6xl pointer-events-auto">
        ...
        {mobileMenuOpen && (
          <div
            className={`mt-2 p-5 rounded-3xl border shadow-2xl lg:hidden flex flex-col gap-2 ${
              isLight ? 'bg-ls-white/95 border-ls-grey-dark/30 text-ls-navy' : 'bg-ls-navy/95 border-ls-white/15 text-ls-white'
            }`}
          >
            {NAV_LINKS.map((link) => (
              <Link
                ...
                to={link.href}
                onClick={() => setMobileMenuOpen(false)}
                className={`ripple-on px-3 py-3 rounded-xl text-sm font-bold tracking-wider transition-colors ${
                  isLight ? 'text-ls-navy hover:bg-ls-navy/5 hover:text-ls-red' : 'text-ls-white hover:bg-ls-white/5 hover:text-ls-red'
                }`}
              >
                {link.name}
              </Link>
