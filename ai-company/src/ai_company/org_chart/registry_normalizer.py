...
class Department(BaseModel):
    ...
    critical_skills: List[str] = Field(
        default_factory=list, description="Critical skills for department"
    )
    ...
    last_updated: str = Field(
        default_factory=datetime.now().isoformat, description="Last update timestamp"
    )
...
class RegistryNormalizer:
    ...
    def build_reporting_chains(
        self, registry_data: List[Dict[str, Any]], dept_context: Dict[str, Any]
    ) -> List[ReportingChain]:
    ...
    def _calculate_depth(self, agent_id: str, registry_data: List[Dict[str, Any]]) -> int:
        ...
        while current:
            ...
            if (
                not parent
                or parent == "CEO"
                or not any(a.get("id") == parent for a in registry_data)
            ):
                break
    ...
    def _calculate_performance_rating(
        self, agent: Dict[str, Any], dept_context: Dict[str, Any]
    ) -> float:
    ...
    def create_unified_structure(self, registry_data: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        for dept_name, dept_info in dept_context["departments"].items():
            ...
            dept_agents = [agent for agent in registry_data if agent.get("department") == dept_name]
            ...
            dept_info["executive_name"] = next(
                (
                    ...
                    if agent.get("id") == dept_info["executive"]
                ),
                dept_info["executive"],
            )
    ...
    def _calculate_dept_avg_performance(
        self, dept_agents: List[Dict[str, Any]], dept_context: Dict[str, Any]
    ) -> float:
    ...
    def _create_department_summary(self) -> Dict[str, Dict[str, Any]]:
        ...
        for dept_name, _dept_info in self.departments.items():
            ...
            summary[dept_name] = {
                ...
                "avg_capacity": sum(c.capacity for c in dept_chains) / len(dept_chains)
                if dept_chains
                else 0,
                "avg_performance": sum(c.performance_rating for c in dept_chains) / len(dept_chains)
                if dept_chains
                else 0,
                "critical_skills": list(
                    set(skill for c in dept_chains for skill in c.critical_skills)
                ),
                "succession_risk_count": len(
                    [c for c in dept_chains if c.succession_risk in ["high", "medium"]]
                ),
            }
    ...
    def validate_data_consistency(self, unified_structure: Dict[str, Any]) -> Dict[str, List[str]]:
        ...
        for agent in self.reporting_chains.values():
            if agent.department and agent.department not in self.departments:
                errors.append(
                    f"Agent {agent.agent_id} references unknown department: {agent.department}"
                )
