...
## 8 Agent ID Naming Convention (CEO Directive — 2026-07-24)

> **Hyphens (`-`) are the ONLY canonical format for agent IDs.**

| Format | Example | Status |
|--------|---------|--------|
| `cloud-architect` | `@cloud-architect` | ✅ **Canonical** |
| `cloud_architect` | `@cloud_architect` | ❌ **Deprecated** — do not use |

**Rules:**
- All agent IDs in `company-registry.yaml` use hyphens.
- All references in documentation, task delegation, and user interactions must use hyphens.
- Underscore variants are non-standard and will not be supported.
- When adding new agents, always use hyphen-separated multi-word IDs (e.g., `qa-automation-engineer`, NOT `qa_automation_engineer`).


## 10. Pre-Commit Validation for Agent IDs

### P2-01 — Validate hyphen-only format in company-registry.yaml

- **Status:** 🟡 PLANNED — Implementation in Sprint 4
- **Owner:** lead-backend
- **Description:** Add a pre-commit hook that validates all agent IDs in company-registry.yaml use hyphen-only format (no underscores).

### P2-02 — Validate generated filenames match registry entries

- **Status:** 🟡 PLANNED — Implementation in Sprint 4
- **Owner:** lead-backend
- **Description:** Add a pre-commit hook that verifies generated filenames match registry entries exactly (kebab-case, no underscores).

### P2-03 — CLI type hints and docstrings

- **Status:** 🔴 STARTED — Type hints being added to CLI modules
- **Owner:** lead-backend
- **Description:** Add type hints to all public CLI functions and docstrings.

This work is documented in docs/REMAINING-WORK-INVENTORY.md P3 items 9.3-9.4 and docs/SPRINT-4-BACKLOG.md P2-03.
