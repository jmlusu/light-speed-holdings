...
description: "Builds distinctive, on-brand LightSpeed websites, landing pages, and web apps instead of generic AI-generated pages. Enforces the LightSpeed design system (navy/red/cyan, Arial type scale, 4px grid), visual hierarchy, responsive behavior, and the no-AI-slop checklist. Targets the repo's existing framework: the repo-root Vite React SPA (React + Tailwind 4 + framer-motion + lucide + recharts). QA via Playwright screenshots -> ls-artifact-qa. Trigger on: 'build a website', 'landing page', 'redesign the site', 'frontend', 'make a page for X', '/create-website'."
---
...
2. Build in the repo-root Vite React SPA: `npm run dev` (background mode: `npm run dev`), `npm run build`, `npm run lint`. Deployed via Vercel (`vercel.json` → `dist`).
