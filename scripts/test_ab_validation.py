...
def generate_synthetic_data():
    ...
    for i in range(100):
        ...
        agent_name = random.choice(
            ["lead-backend", "lead-frontend", "senior-backend", "specialist-api"]
        )
        ...
        for exp_name in framework._experiments:
            ...
            if variant == "control":
                ...
                total_tokens = int(
                    random.gauss(
                        control_base_tokens * (1 - treatment_reduction), control_variance * 0.5
                    )
                )
...
def run_validation():
    ...
    result = subprocess.run(
        [
            sys.executable,
            "scripts/validate_token_optimization.py",
            "--days",
            "1",
            "--format",
            "text",
