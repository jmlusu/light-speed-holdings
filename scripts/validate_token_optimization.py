...
def compare_variants(
    control: list[ExperimentMetrics], treatment: list[ExperimentMetrics]
) -> dict[str, Any]:
    ...
    token_reduction = (
        ...
        / control_stats["tokens"]["mean"]
        * 100
    )
    cost_reduction = (
        (
            (control_stats["cost_usd"]["mean"] - treatment_stats["cost_usd"]["mean"])
            / control_stats["cost_usd"]["mean"]
            * 100
        )
        if control_stats["cost_usd"]["mean"] > 0
        else 0
    )
    iteration_reduction = (
        (
            (control_stats["iterations"]["mean"] - treatment_stats["iterations"]["mean"])
            / control_stats["iterations"]["mean"]
            * 100
        )
        if control_stats["iterations"]["mean"] > 0
        else 0
    )
    duration_change = (
        (
            (
                treatment_stats["duration_seconds"]["mean"]
                - control_stats["duration_seconds"]["mean"]
            )
            / control_stats["duration_seconds"]["mean"]
            * 100
        )
        if control_stats["duration_seconds"]["mean"] > 0
        else 0
    )
    ...
    return {
        ...
        "quality": {
            ...
            "quality_maintained": success_diff >= -0.05
            and completion_diff >= -0.05,  # Allow 5% degradation
        },
        "sample_sizes": {
...
def generate_report(
    experiment_names: list[str] | None = None, days: int = 7, output_format: str = "text"
) -> dict[str, Any]:
...
def print_text_report(report: dict[str, Any]) -> None:
    ...
    for exp_name, data in report["experiments"].items():
        ...
        print("  TOKEN USAGE:")
        print(
            f"    Control:      {c['tokens']['mean']:,.0f} tokens (median: {c['tokens']['median']:,.0f})"
        )
        print(
            f"    Treatment:    {t['tokens']['mean']:,.0f} tokens (median: {t['tokens']['median']:,.0f})"
        )
        print(f"    Reduction:    {r['token_reduction_pct']:+.1f}%")
        print()
        print("  COST (USD):")
        print(
            f"    Control:      ${c['cost_usd']['mean']:.6f} (total: ${c['cost_usd']['total']:.4f})"
        )
        print(
            f"    Treatment:    ${t['cost_usd']['mean']:.6f} (total: ${t['cost_usd']['total']:.4f})"
        )
        print(f"    Reduction:    {r['cost_reduction_pct']:+.1f}%")
        print()
        print("  ITERATIONS:")
        print(
            f"    Control:      {c['iterations']['mean']:.1f} (median: {c['iterations']['median']:.1f})"
        )
        print(
            f"    Treatment:    {t['iterations']['mean']:.1f} (median: {t['iterations']['median']:.1f})"
        )
        ...
        print("  QUALITY:")
        print(
            f"    Success rate:     Control={c['success_rate'] * 100:.1f}%, Treatment={t['success_rate'] * 100:.1f}% ({q['success_rate_diff']:+.1f}%)"
        )
        print(
            f"    Completion rate:  Control={c['completion_rate'] * 100:.1f}%, Treatment={t['completion_rate'] * 100:.1f}% ({q['completion_rate_diff']:+.1f}%)"
        )
        ...
        print(
            f"  TARGET (50% token reduction): {'MET' if target_met else 'NOT MET'} ({r['token_reduction_pct']:.1f}%)"
        )
...
def main():
    ...
    parser.add_argument(
        "--target", type=float, default=50.0, help="Token reduction target percentage"
    )
